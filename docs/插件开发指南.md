...
2. **迁移插件 (Migration Plugins)** - 数据库结构变更
...
migrate(
    // up 迁移
    (app) => {
        console.log("执行向上迁移: add_user_preferences");
        ...
        console.log("用户偏好设置表创建完成");
    },

    // down 回滚
    (app) => {
        console.log("执行回滚迁移: add_user_preferences");

        // 删除表
        app.db.run("DROP TABLE IF EXISTS user_preferences");

        console.log("用户偏好设置表删除完成");
    }
);
