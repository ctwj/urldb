...
// hooksBinds 钩子绑定
func hooksBinds(app core.App, vm *goja.Runtime, executors *pool) {
	vm.Set("onURLAdd", func(handler goja.Value) {
		if goja.IsFunction(handler) {
			...
			app.OnURLAdd().BindFunc(func(e *core.URLEvent) error {
				...
				// 调用JavaScript处理器
				_, err := vm.CallFunction(handler.(goja.Callable), nil, e.URL, e.Data)
				if err != nil {
	...
	vm.Set("onUserLogin", func(handler goja.Value) {
		if goja.IsFunction(handler) {
			app.OnUserLogin().BindFunc(func(e *core.UserEvent) error {
				...
				_, err := vm.CallFunction(handler.(goja.Callable), nil, e.User, e.Data)
				if err != nil {
					utils.Error("JavaScript hook error: %v", err)
				}
...
// cronBinds 定时任务绑定
func cronBinds(app core.App, vm *goja.Runtime, executors *pool) {
...
// routerBinds 路由绑定
func routerBinds(app core.App, vm *goja.Runtime, executors *pool) {
	vm.Set("router", map[string]interface{}{
		"add": func(method, path string, handler goja.Value) error {
			if goja.IsFunction(handler) {
				// TODO: 实现路由注册
				utils.Info("Route registered: %s %s", method, path)
			}
			return nil
		},
	})
